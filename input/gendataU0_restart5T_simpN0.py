...
runname = 'TideU0%02dN0SimH%03dho%03dAh%04dKh%04dCdqdt003_advSch2/' % (100*u0,H,h0,10000*Ah,10000*Kh)
comments = ''
...
##### make sure the initial field come from ####
read_dir='/scratch/jxchang/HighRes1/results/TideU008N0SimH200ho140Ah0200Kh0200Cdqdt003/python/'

# initial field
# temperature profile...
readT = pd.read_csv(read_dir+"T_5T.csv")
T0_5T= np.reshape(readT.THETA.to_numpy(),((nz,ny,nx)))

#shape is [nz,ny,nx]
with open(indir+"/T0_5T.bin", "wb") as f:
	T0_5T.tofile(f)
f.close()
print(T0_5T)
print('shpe of T0' + str(np.shape(T0_5T)) )


# U profile...
readU = pd.read_csv(read_dir+"U_5T.csv")
U0_5T= np.reshape(readU.UVEL.to_numpy(),((nz,ny,nx)))
readV = pd.read_csv(read_dir+"V_5T.csv")
V0_5T= np.reshape(readV.VVEL.to_numpy(),((nz,ny,nx)))

with open(indir+"/U0_5T.bin", "wb") as f:
	U0_5T.tofile(f)
f.close()
print(U0_5T)
print('shpe of U0' + str(np.shape(U0_5T)) )

with open(indir+"/V0_5T.bin", "wb") as f:
	V0_5T.tofile(f)
f.close()
print(V0_5T)
print('shpe of V0' + str(np.shape(V0_5T)) )

#ETA profile
readE = pd.read_csv(read_dir+"Eta_5T.csv")
Eta0_5T= np.reshape(readE.ETAN.to_numpy(),(ny,nx))

with open(indir+"/Eta0_5T.bin", "wb") as f:
	Eta0_5T.tofile(f)
f.close()
print(Eta0_5T)
print('shpe of Eta0' + str(np.shape(Eta0_5T)) )
...
rhoNil=999.8
N_new= np.zeros(nz)
N_new= 0.020866*np.exp(-z/100)-0.002656
