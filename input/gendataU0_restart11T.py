...
runname = 'TideU0%02dN0%03dH%03dho%03dAh%04dKh%04dCdqdt003_advSch2/' % (100*u0,1000*N0,H,h0,10000*Ah,10000*Kh)
comments = ''
...
read_dir='~/scratch/HighRes2/results/TideU008N0LinH200ho140Ah0200Kh0200Cdqdt003/reduceddata/'

# initial field
# temperature profile...
readT = pd.read_csv(read_dir+"T_11T.csv")
T0_11T= np.reshape(readT.THETA.to_numpy(),((nz,ny,nx)))

#shape is [nz,ny,nx]
with open(indir+"/T0_11T.bin", "wb") as f:
	T0_11T.tofile(f)
f.close()
print(T0_11T)
print('shpe of T0' + str(np.shape(T0_11T)) )


# U profile...
readU = pd.read_csv(read_dir+"U_11T.csv")
U0_11T= np.reshape(readU.UVEL.to_numpy(),((nz,ny,nx)))
readV = pd.read_csv(read_dir+"V_11T.csv")
V0_11T= np.reshape(readV.VVEL.to_numpy(),((nz,ny,nx)))

with open(indir+"/U0_11T.bin", "wb") as f:
	U0_11T.tofile(f)
f.close()
print(U0_11T)
print('shpe of U0' + str(np.shape(U0_11T)) )

with open(indir+"/V0_11T.bin", "wb") as f:
	V0_11T.tofile(f)
f.close()
print(V0_11T)
print('shpe of V0' + str(np.shape(V0_11T)) )

#ETA profile
readE = pd.read_csv(read_dir+"Eta_11T.csv")
Eta0_11T= np.reshape(readE.ETAN.to_numpy(),(ny,nx))

with open(indir+"/Eta0_11T.bin", "wb") as f:
	Eta0_11T.tofile(f)
f.close()
print(Eta0_11T)
print('shpe of Eta0' + str(np.shape(Eta0_11T)) )
