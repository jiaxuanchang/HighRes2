...
Kh = 2.0e-2
Cd = 1.0e-3
a2 = 200.**2 #(HEADLAND WIDTH)

########### change everytime compile !!!!!!!!!!!!
runname = 'TideU0%02dsinN0SimH%03dho%03dGauRidParHeadAh%04dKh%04da240000_freeslipBotSide_Cdqdt%03d/' % (100*u0,H,h0,10000*Ah,10000*Kh,1000*Cd)
comments = ''
...
if backupmodel:
    ...
    copy('./gendataU0_GausRid_ParabHead_simpN0.py',outdir)
else:
      outdir=outdir+'input/'

outdir_p = '/home/jxchang/projects/def-jklymak/jxchang/HighRes2/input/'
print('outdir_p:'+str(outdir_p))
copy('./gendataU0_GausRid_ParabHead_simpN0.py',outdir_p)
...
if is2D==0:
    ...
    topowallN=10*(500-(xx**2/a2)+(yy-1487))
    topowallN[topowallN>0]=0
    topowallN[topowallN<-H]=-H
    print(np.shape(topowallN))

    topowallS=10*(500-(xx**2/a2)-(yy+1487))
    topowallS[topowallS>0]=0
    topowallS[topowallS<-H]=-H
...
if 1:
    ...
    savefig(outdir+'/figs/topo3d_zoom.png')
...
#N_new= 0.033*np.exp(-z/53+25/53)-0.0028 N0Sim011382
#N_new[0:6]=0.013 #top const N
N_new[-10:]=0.002
#N_new[-12:]=0.0011 N0Sim011382
...
#time1 = arange(0,4.*3720.,dt)
#time2 = arange(4.*3720,12.*3720.,dt)
#print(time/3600./12.4)
om = 2*pi/12.40/3600;
uw = u0*np.sin(om*time)
ue = u0*np.sin(om*time)
#uw2 = u0*np.ones(len(time2))
#ue2 = u0*np.ones(len(time2))
#uw=np.concatenate((uw1,uw2))
#ue=np.concatenate((ue1,ue2))
